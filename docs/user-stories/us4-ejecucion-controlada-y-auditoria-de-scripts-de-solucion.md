...
* **Persistencia:** Captura el flujo de salida estándar (`stdout`) y el flujo de errores (`stderr`) como buffers independientes, persistidos en las columnas `stdout_log`/`stderr_log` de la tabla `remediation_actions` (`LOG-US4-BE-02B`, decisión aprobada 2026-08-11 — reemplaza el diseño original de un único campo combinado `execution_log`), para que el frontend pueda renderizar cada stream sin heurísticas de parseo de texto.
